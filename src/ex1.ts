...
function isFromFibonacci(input: number): boolean {
  if (
...
function computeNorFiboNorPrime(maxSize: number) {
  let i: number = 0;
  let size: number = norFiboNorPrime.length;
  while (size < maxSize) {
    let prime: boolean = isPrime(i);
    let fibo: boolean = isFromFibonacci(i);
    if (!prime && !fibo) {
